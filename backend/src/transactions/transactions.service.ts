...
export class TransactionsService {
  ...
  async create(dto: CreateTransactionDto) {
    if (!dto.detalles || dto.detalles.length === 0) {
      throw new BadRequestException('La transacción debe tener al menos un detalle');
    }
    ...
    for (const det of dto.detalles) {
      if (det.usuarioEmisorId === det.usuarioReceptorId) {
        throw new BadRequestException('Un usuario no puede solicitar sus propias publicaciones');
      }
      ...
      if (!publicacion) {
        throw new NotFoundException(`La publicación con ID ${det.publicacionId} no existe o no está activa`);
      }

      if (publicacion.user.id !== det.usuarioEmisorId) {
        throw new BadRequestException(`La publicación ${det.publicacionId} no pertenece al usuario emisor ${det.usuarioEmisorId}`);
      }

      if (publicacion.expirationDate <= new Date()) {
        throw new BadRequestException(`La publicación ${det.publicacionId} está vencida y no puede ser transada`);
      }

      if (publicacion.cantidad < det.cantidad) {
        throw new BadRequestException(`La publicación ${det.publicacionId} no tiene stock suficiente. Disponible: ${publicacion.cantidad}`);
      }

      const emisor = await this.userRepository.findOne({ where: { id: det.usuarioEmisorId } });
      const receptor = await this.userRepository.findOne({ where: { id: det.usuarioReceptorId } });

      if (!emisor || !receptor) {
        throw new NotFoundException('Uno o más usuarios de la transacción no existen');
      }
  ...
  async findAll() {
    return await this.transactionRepository.find({
      relations: ['detalles', 'detalles.usuarioEmisor', 'detalles.usuarioReceptor', 'detalles.publicacion'],
    });
  }
  ...
  async updateStatus(id: number, dto: UpdateTransactionDto) {
    ...
    if (dto.estado === TransactionState.COMPLETED) {
      for (const det of transaccion.detalles) {
        const pub = await this.publicationRepository.findOne({ where: { id: det.publicacion.id } });
        if (pub && pub.cantidad >= det.cantidad) {
          ...
          throw new BadRequestException(`No hay stock suficiente para completar el trueque de la publicación ${det.publicacion.id}`);
        }
      }
    }
  ...
  async remove(id: number) {
    ...
    if (transaccion.estado === TransactionState.COMPLETED) {
      throw new BadRequestException('No se puede cancelar una transacción que ya fue completada');
    }
  ...
  async rateTransaction(id: number, dto: RateTransactionDto) {
    ...
    if (transaccion.estado !== TransactionState.COMPLETED) {
      throw new BadRequestException('Solo se pueden calificar transacciones finalizadas/completadas');
    }
